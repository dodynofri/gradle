...
"""
    ...
    List<TestFile> outputDirs(String from, String to) {
        List<TestFile> dirs = []
        def baseDir = new TestFile(cacheDir, "files-1.1", from).absolutePath + File.separator
    ...
    TestFile getGcFile() {
        return new TestFile(cacheDir, "gc.properties")
    }
    ...
    void markForCleanup(File file) {
        file.lastModified = System.currentTimeMillis() - DAYS.toMillis(MAX_CACHE_AGE_IN_DAYS) * 2
    }
